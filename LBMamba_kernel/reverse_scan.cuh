...
/**
 * Added!!!!!!!!!!!!!!!!!!!!!!!
 * Perform a sequential reverse scan (inclusive scan - X) over the statically-sized \p input array, seeded with the specified \p postfix.  The aggregate is returned.
 */
template <
    int       LENGTH,
    typename  T> // must be float2
__device__ __forceinline__ float ThreadReverseScanForForward(
    const T (&scan_input)[LENGTH], // input scan sequence, we use A in this sequence
    float (&output)[LENGTH], // output sequence in float
    const float postfix)
{
    // Careful, output maybe be aliased to input
    static_assert(std::is_same_v<T, float2>);
    float exclusive = postfix;
    #pragma unroll
    for (int i = LENGTH - 1; i >= 0; --i) {
        output[i] = scan_input[i].x * exclusive;
        exclusive = output[i] + scan_input[i].y;
    }
    return exclusive;
}
...
/**
 * Added for bwd kernel!!!!!!!!!!!!!!!!!!!!!!!
 */
template <int LENGTH>
__device__ __forceinline__ void ThreadReverseScanHRForBackward(
    const float2 (&data)[LENGTH],
    float (&inout)[LENGTH],
    const float postfix)
{
    float inclusive = postfix;

    #pragma unroll
    for (int i = LENGTH - 1; i >= 0; --i) {
        inclusive = inclusive * data[i].x + data[i].y;
        inout[i] = inclusive;
    }
}
...
// */
//template <int LENGTH>
//__device__ __forceinline__ void ThreadScanHRForBackward(
//    const float2 (&data)[LENGTH],
//    float (&inout)[LENGTH],
//    const float prefix)
//{
//    float inclusive = prefix;
//
//    #pragma unroll
