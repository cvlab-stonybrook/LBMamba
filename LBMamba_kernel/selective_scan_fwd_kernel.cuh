    ...
    for (int chunk = 0; chunk < params.n_chunks; ++chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                // Added !!!!!!!!!!!!!!!!!!!!
                // locally reverse scan
                float thread_reverse_data[kNItems];

                if constexpr (!kIsComplex) {
//                     #pragma unroll
//                     for (int i = 0; i < kNItems; ++i) {
//                         thread_reverse_data[i] = thread_data[i].y;
//                     }
                    ThreadReverseScanForForward<kNItems, scan_t>(thread_data, thread_reverse_data, 0.0f);
                } else {
                    // Not implemented
                }
                ...
                // Added merge reverse scan values
                // locally reverse scan
                if constexpr (!kIsComplex) {
                    #pragma unroll
                    for (int i = 0; i < kNItems; ++i) {
                        thread_data[i].y = thread_data[i].y + thread_reverse_data[i];
                    }
                } else {
                    // Not implemented
                }
        ...
        for (int r = 0; r < kNRows; ++r) {
            ...
            store_output1<Ktraits>(out + r * params.out_d_stride, out_vals[r], smem_store1, params.seqlen - chunk * kChunkSize);
        }
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                    using Ktraits = Selective_Scan_fwd_kernel_traits<
                            kNThreads, kNItems, kNRows, kIsEvenLen, kIsVariableB, kIsVariableC, kHasZ,
