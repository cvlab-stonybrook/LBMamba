...
struct Selective_Scan_bwd_kernel_traits {
    ...
    static constexpr int kMinBlocks = kNThreads == 128 && !kIsComplex ? 2 : 2;
...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk)
    {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            // 03/26/2025 LocalBidirMamba
            scan_t thread_hr_reverse_data[kNItems];
            ...
            if constexpr (!kIsComplex) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    // 03/26/2025 LocalBidirMamba
                    if (i == kNItems - 1) {
                        // do nothing, do not need to dump this value
                        // Assign value thread_hr_reverse_data[0].x = 1.0f; later
                    } else {
                        thread_hr_reverse_data[i + 1].x = delta_a_exp;
                    }
                    ...
                    // 03/26/2025 LocalBidirMamba
                    thread_hr_reverse_data[i].y = thread_reverse_data[i].y;
                }
                __syncthreads();
                ...
                // 03/26/2025 LocalBidirMamba
                thread_hr_reverse_data[0].x = 1.0f;

                #ifdef DEBUG_HR_CONSTRUCTION
                float thread_reverse_data_2[kNItems];

                if constexpr (!kIsComplex) {
                    #pragma unroll
                    for (int i = 0; i < kNItems; ++i) {
                        thread_reverse_data_2[i] = thread_data[i].y;
                    }
                    ThreadReverseScanForForward<kNItems, scan_t>(thread_data, thread_reverse_data_2, 0.0f);
                } else {
                    // Not implemented
                }
                #endif  // DEBUG_HR_CONSTRUCTION

                // reconstruct h^R, (in-thread right-to-left scan, same as the forward + \Delta_t B_t u_t)
                float thread_hr_data[kNItems];
//                 #pragma unroll
                // for (int i = 0; i < kNItems; ++i) { thread_hr_data[i] = thread_data[i].y; }
                ThreadReverseScanHRForBackward<kNItems>(thread_data, thread_hr_data, 0.0f);
                ...
                // thread-level right inverse scan, (in-thread grad scan, left-to-right)
                #ifdef DEBUG_NAN_DB_VALS
                if (threadIdx.x == 0)
                {
                    for (int i = 0; i < kNItems; ++i)
                    {
                        printf("Block %d Thread 0 item %d, BEFORE THREAD SCAN "
                               "thread_hr_reverse_data[%d] (%f, %f) "
                               "thread_reverse_data[%d] (%f, %f)\n",
                               blockIdx.x, i,
                               i, thread_hr_reverse_data[i].x, thread_hr_reverse_data[i].y,
                               i, thread_reverse_data[i].x, thread_reverse_data[i].y);
                    }
                }
                #endif  // DEBUG_NAN_DB_VALS

                cub::internal::ThreadScanInclusive(thread_hr_reverse_data, thread_hr_reverse_data, SSMScanOp<weight_t>());
...
//                    const float dx = thread_reverse_data[i].y;
                    const float dx_left = thread_reverse_data[i].y;
                    const float dx_right = thread_hr_reverse_data[i].y;

//                    const float ddelta_u = !kIsVariableB ? dx : dx * B_vals[i];
                    // We have " - \Delta_t B_t u_t "
                    float ddelta_u = FP_NAN;

                    if constexpr (kIsVariableC)
                    {
                        ddelta_u = !kIsVariableB
                                ? dx_left + dx_right - dout_vals[i] * C_vals[i]
                                : (dx_left + dx_right - dout_vals[i] * C_vals[i]) * B_vals[i];
                    }
                    else
                    {
                        printf("const C not implemented!!!");
                        __trap();
                    }
...
//                    const float a = thread_data[i].y - (!kIsVariableB ? delta_vals[i] * float(u_vals[i]) : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    const float a_left = thread_data[i].y -
                            (!kIsVariableB
                            ? delta_vals[i] * float(u_vals[i])
                            : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    const float a_right = thread_hr_data[i] -
                            (!kIsVariableB
                            ? delta_vals[i] * float(u_vals[i])
                            : delta_vals[i] * float(u_vals[i]) * B_vals[i]);
                    ...
                    ddelta_vals[i] +=
                            ...
                            dx_left * A_val * a_left + dx_right * A_val * a_right;                // <-- for delta's ops on A
                    ...
                    //        == dx * a * delta_vals[i]
                    //        ==  ↓
                    dA_val += delta_vals[i] * (dx_left * a_left + dx_right * a_right);
                    //dA_val += delta_vals[i] * dx_left * a_left;
                    //dA_val += delta_vals[i] * dx_right * a_right;

                    // 03/26/2025 Modified by LocallyBidirMamba.
                    if constexpr (!kIsVariableB || !kIsVariableC) {
                        if constexpr (!kIsVariableB) {  // B const, dBC_val is dB_val
                            const float H = thread_data[i].y + thread_hr_data[i] - delta_vals[i] * float(u_vals[i]);
                            dBC_val += dout_vals[i] * (!kIsVariableC ? H : H * C_vals[i]);
                        } else {  // dBC_val is dC_val
                            const float H = thread_data[i].y + thread_hr_data[i] - delta_vals[i] * float(u_vals[i]) * B_vals[i];
                            dBC_val += dout_vals[i] * H;
                        }
                    }
                    ...
                    if constexpr (kIsVariableB)
                    {
                        if constexpr (kIsVariableC)
                        {
                            dB_vals[i] = (dx_left + dx_right - dout_vals[i] * C_vals[i]) * delta_vals[i] * float(u_vals[i]);
                        }
                        else
                        {
                            printf("var B const C not implemented!!!\n");
                            __trap();
                        }
                        ...
                        if (threadIdx.x == 0)
                        {
                            printf("Block %d Thread 0 item %d, "
                                   "dx_left %f, dx_right %f, "
                                   "thread_hr_reverse_data[%d] (%f, %f)\n",
                                   blockIdx.x, i,
                                   dx_left, dx_right,
                                   i, thread_hr_reverse_data[i].x, thread_hr_reverse_data[i].y);
                    ...
                    if constexpr (kIsVariableC) {
                        if constexpr (!kIsVariableB) {
                            ...
                            #ifdef DEBUG_HR_CONSTRUCTION
                            const float H2 = thread_data[i].y + thread_reverse_data_2[i];
                            if (1e-4f < abs(H - H2) && threadIdx.x == 0) {
                                printf("Block %d Thread 0 i %d, H == %f + %f - %f == %f, H2 == %f + %f == %f\n",
                                       blockIdx.x, i,
                                       thread_data[i].y, thread_hr_data[i], delta_vals[i] * float(u_vals[i]) * B_vals[i], H,
                                       thread_data[i].y, thread_reverse_data_2[i], H2);
                            }
                            #endif  // DEBUG_HR_CONSTRUCTION
            ...
            // Complex
                ...
                printf("complex_t not supported!!!\n");
                __trap();
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                ...
                BOOL_SWITCH(params.delta_softplus, kDeltaSoftplus, [&] {
                    BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                        using Ktraits = Selective_Scan_bwd_kernel_traits<
                                kNThreads, kNItems, kIsEvenLen, kIsVariableB, kIsVariableC, kDeltaSoftplus, kHasZ,
