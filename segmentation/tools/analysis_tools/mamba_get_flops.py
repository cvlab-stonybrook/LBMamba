...
#     # 2.2 x_proj
#     flops += L * layer.mixer.x_proj.in_features * layer.mixer.x_proj.out_features
#     # 2.3 dt_proj
#     flops += L * layer.mixer.dt_proj.in_features * layer.mixer.dt_proj.out_features
...
def MambaInnerFnNoOutProj_jit(inputs, outputs):
    ...
    # https://github.com/state-spaces/mamba/issues/110
    coeff = 9 #12 if lbm else 9 # only vim use this function
    flops = coeff * Batch * L * Dim * H
    if with_D:
        flops += Batch * Dim * L
    if with_Z:
        flops += Batch * Dim * L
...
def MambaInnerFn_jit(inputs, outputs):
    ...
    lbm=True
    if not lbm:
        xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, out_proj_weight, out_proj_bias, A = inputs[
                                                                                                              0:8]
    else:
        xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight, out_proj_weight, A = inputs[0:7]
    Batch, _, L = xz.type().sizes()
    CWidth = conv1d_weight.type().sizes()[-1]
    H = A.type().sizes()[-1]  # 16
    Dim, R = delta_proj_weight.type().sizes()
    ...
    # https://github.com/state-spaces/mamba/issues/110
    coeff = 12 #if lbm else 9 # only lbm use this function
    flops = coeff * Batch * L * Dim * H
    if with_D:
        flops += Batch * Dim * L
    if with_Z:
...
# MambaInnerFnNoOutProj_flop_jit: Callable = MambaInnerFnNoOutProj_jit
...
def mmengine_flop_count(model: nn.Module = None, input_shape = (3, 224, 224), show_table=False, show_arch=False, _get_model_complexity_info=False):
    ...
    def get_model_complexity_info(
        ...
        if model.backbone.__class__.__name__ == 'MM_BiVim':
            ...
            supported_extra_ops_["prim::PythonOp.MambaInnerFnNoOutProj"] = MambaInnerFnNoOutProj_jit
            supported_extra_ops_["prim::PythonOp.MambaInnerFn"] = MambaInnerFn_jit
