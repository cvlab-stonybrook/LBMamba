...
import models_mamba_bi_cls # does not work
...
def get_args_parser():
    ...
    parser.add_argument('--small_eval_bs', action='store_true', default=False)
    # add save_ckpt_freq
    parser.add_argument('--save_ckpt_freq', default=0, type=int, help='save checkpoint frequency')
    return parser
...
def main(args):
    ...
    batch_size_eval = int(3 * args.batch_size) if args.small_eval_bs else int(1.5 * args.batch_size * 20)
    ...
    while True:
        if collapse:
            print("Model collapse, resuming from previous epoch")
            checkpoint = previous_weight
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.output_dir:
                ...
                if args.save_ckpt_freq > 0 and (epoch + 1) % args.save_ckpt_freq == 0:
                    checkpoint_paths = [output_dir / ('checkpoint_%d.pth' % epoch)]
                    for checkpoint_path in checkpoint_paths:
                        print("saving to checkpoint_%d.pth" % epoch)
                        utils.save_on_master({
                            'model': model_without_ddp.state_dict(),
                            'optimizer': optimizer.state_dict(),
                            'lr_scheduler': lr_scheduler.state_dict(),
            ...
            if epoch > 5 and (test_stats["acc1"] < 10 
                              or (max_accuracy > 5 and test_stats["acc1"] < max_accuracy - 20)):
                # collapse, need to restart
                collapse = True
                break
            else:
                previous_weight = {
