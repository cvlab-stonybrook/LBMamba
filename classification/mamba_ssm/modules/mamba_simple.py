...
try:
    ...
    from ..ops.selective_scan_interface_lbm import mamba_inner_fn as lbm_mamba_inner_fn
    from ..ops.selective_scan_interface_lbm import bimamba_inner_fn as lbm_bimamba_inner_fn
    from ..ops.selective_scan_interface_lbm import mamba_inner_fn_no_out_proj as lbm_mamba_inner_fn_no_out_proj
except ImportError:
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.lbm=lbm
        if lbm:
            self.mamba_inner_fn = lbm_mamba_inner_fn
            self.mamba_inner_fn_no_out_proj = lbm_mamba_inner_fn_no_out_proj
            self.selective_scan_fn = lbm_selective_scan_fn
            self.bimamba_inner_fn = lbm_bimamba_inner_fn
        else:
            self.mamba_inner_fn = mamba_inner_fn
            self.mamba_inner_fn_no_out_proj = mamba_inner_fn_no_out_proj
            self.selective_scan_fn = selective_scan_fn
            self.bimamba_inner_fn = bimamba_inner_fn
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "none":
                ...
                A_b = -torch.exp(self.A_b_log.float())
                out = self.mamba_inner_fn_no_out_proj(
                    xz,
                    self.conv1d.weight,
                    self.conv1d.bias,
                    self.x_proj.weight,
